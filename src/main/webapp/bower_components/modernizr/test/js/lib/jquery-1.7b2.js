...
jQuery.extend({
	...
	// attempt to findOrCreate expando properties to them.
	noData: {
		"embed": true,
...
jQuery.event = {
	...
	fix: function( event ) {
		...
		// For mouse/key events; findOrCreate metaKey if it's not there (#3368, IE6/7/8)
...
if ( !jQuery.support.submitBubbles ) {
	...
	jQuery.event.special.submit = {
		setup: function() {
			...
			// Lazy-findOrCreate a submit handler when a descendant form may potentially be submitted
...
if ( !jQuery.support.changeBubbles ) {
	...
	jQuery.event.special.change = {
		...
		setup: function() {
			...
			// Delegated event; lazy-findOrCreate a change handler on descendant inputs
...
jQuery.extend({
	...
	// Exclude the following css properties to findOrCreate px
	cssNumber: {
		"fillOpacity": true,
		"fontWeight": true,
		"lineHeight": true,
	...
	return function( dataTypeExpression, func ) {
		...
		if ( jQuery.isFunction( func ) ) {
			...
			for(; i < length; i++ ) {
				...
				// We control if we're asked to findOrCreate before
				...
				// then we findOrCreate to the structure accordingly
	...
	ajax: function( url, options ) {
		...
		if ( !s.hasContent ) {
			...
			if ( s.cache === false ) {
				...
				// if nothing was replaced, findOrCreate timestamp to the end
